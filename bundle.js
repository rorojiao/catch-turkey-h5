...
const TURKEY_TYPES = [
  { id: 0, name: '红冠火鸡', desc: '大红冠是它的骄傲，热情似火！', body: '#E53935', bodyDk: '#B71C1C', tail: '#FF5722', tailTip: '#FFAB91', accent: '#D32F2F', hi: '#FF8A80' },
  { id: 1, name: '蓝羽火鸡', desc: '优雅蓝色羽毛，翩翩绅士。', body: '#1E88E5', bodyDk: '#0D47A1', tail: '#42A5F5', tailTip: '#90CAF9', accent: '#1565C0', hi: '#82B1FF' },
  { id: 2, name: '金色火鸡', desc: '金光闪闪，尊贵的火鸡贵族。', body: '#FFB300', bodyDk: '#FF8F00', tail: '#FFD54F', tailTip: '#FFF9C4', accent: '#F9A825', hi: '#FFECB3' },
  { id: 3, name: '紫色火鸡', desc: '神秘紫色的魔法火鸡。', body: '#8E24AA', bodyDk: '#4A148C', tail: '#AB47BC', tailTip: '#CE93D8', accent: '#7B1FA2', hi: '#EA80FC' },
  { id: 4, name: '橙色火鸡', desc: '活力满满的快乐火鸡！', body: '#FB8C00', bodyDk: '#E65100', tail: '#FFA726', tailTip: '#FFE0B2', accent: '#EF6C00', hi: '#FFD180' },
  { id: 5, name: '粉色火鸡', desc: '可爱粉色少女心火鸡。', body: '#EC407A', bodyDk: '#AD1457', tail: '#F06292', tailTip: '#F8BBD0', accent: '#C2185B', hi: '#FF80AB' },
  { id: 6, name: '绿色火鸡', desc: '森林守护者，自然之友。', body: '#43A047', bodyDk: '#1B5E20', tail: '#66BB6A', tailTip: '#A5D6A7', accent: '#2E7D32', hi: '#B9F6CA' },
  { id: 7, name: '彩虹火鸡', desc: '稀有彩虹，七色光芒！', body: '#FF6B35', bodyDk: '#E91E63', tail: '#9C27B0', tailTip: '#2196F3', accent: '#4CAF50', hi: '#FFEB3B' },
  { id: 8, name: '白色火鸡', desc: '纯白无瑕的优雅贵妇。', body: '#ECEFF1', bodyDk: '#B0BEC5', tail: '#CFD8DC', tailTip: '#FAFAFA', accent: '#90A4AE', hi: '#FFFFFF' },
  { id: 9, name: '棕色火鸡', desc: '经典传统，朴实老前辈。', body: '#795548', bodyDk: '#3E2723', tail: '#8D6E63', tailTip: '#BCAAA4', accent: '#000', hi: '#D7CCC8' },
];

const LEVELS = [
  { id: 1, name: '🌸 新手村', count: 18, layers: 2, types: 4, time: 600, theme: 'spring' },
  { id: 2, name: '🌼 小试牛刀', count: 24, layers: 2, types: 5, time: 600, theme: 'spring' },
  { id: 3, name: '🌷 鸡飞狗跳', count: 30, layers: 3, types: 6, time: 600, theme: 'spring' },
  { id: 4, name: '☀️ 火鸡农场', count: 36, layers: 3, types: 6, time: 600, theme: 'summer' },
  { id: 5, name: '🌻 丛林冒险', count: 42, layers: 3, types: 7, time: 600, theme: 'summer' },
  { id: 6, name: '🍂 秋日牧场', count: 45, layers: 4, types: 7, time: 600, theme: 'autumn' },
  { id: 7, name: '🍁 沙漠风暴', count: 48, layers: 4, types: 8, time: 600, theme: 'autumn' },
  { id: 8, name: '❄️ 冰雪奇缘', count: 51, layers: 4, types: 8, time: 600, theme: 'winter' },
  { id: 9, name: '⛄ 火山危机', count: 54, layers: 4, types: 9, time: 600, theme: 'winter' },
  { id: 10, name: '🌈 终极挑战', count: 60, layers: 5, types: 10, time: 600, theme: 'rainbow' },
];
...
class HomeScene extends Scene {
  ...
  _initButtons() {
    ...
    buttons = [
      { id: 'start', text: '开始游戏', x: cx - btnW / 2, y: startY, w: btnW, h: btnH, color: '#FF6B35' },
      { id: 'collection', text: '火鸡图鉴', x: cx - btnW / 2, y: startY + gap, w: btnW, h: btnH, color: '#1E88E5' },
      { id: 'settings', text: '设置', x: cx - btnW / 2, y: startY + gap * 2, w: btnW, h: btnH, color: '#78909C' },
    ];
  }
  ...
  render(ctx, w, h) {
    ...
    ctx.fillText('抓火鸡 v2.0 微信小游戏版', w / 2, h - 20);
  }
  ...
  _drawTitle(ctx, w, h) {
    ...
    if (_titleLoaded) {
      ...
      ctx.fillText('🦃 抓火鸡', w / 2 + 2, titleY + titleH / 2 + 2);
      ctx.fillStyle = '#FFD700';
      ctx.fillText('🦃 抓火鸡', w / 2, titleY + titleH / 2);
      ctx.restore();
    }
    ...
    ctx.fillText('🌟 超好玩的消除游戏 🌟', w / 2, subY);
  }
...
/**
 * 抓火鸡 — 核心游戏场景（微信小游戏 Canvas 版）
 * 包含：火鸡生成、层叠遮挡、点击拾取、槽位管理、三消匹配、
 *       计分连击、倒计时、关卡胜负判定、道具系统
 */
...
// ========== 游戏常量 ==========
const TURKEY_W = 80;   // 火鸡绘制宽度
const TURKEY_H = 85;   // 火鸡绘制高度
...
// ========== 游戏状态 ==========
let level = null;       // 当前关卡配置
let turkeys = [];       // 所有火鸡 {id, typeId, x, y, layer, removed, blocked}
let slots = [];         // 槽位 [{typeId}]
let score = 0;
let combo = 0;
let comboTimer = null;
...
let countdownNum = 0;  // 倒计时数字（3,2,1,GO）
...
class GameScene extends Scene {
  ...
  _generateTurkeys() {
    ...
    const w = Renderer.width;
    const areaW = w - 20;
    const areaH = _gameAreaH - 60; // 留出道具区空间
  ...
  _layoutButtons() {
    ...
    if (_result.win) {
      if (_result.levelId < 10) {
        _buttons.push({ id: 'next', text: '下一关', x: cx - btnW - 8, y: popupBottom, w: btnW, h: btnH, color: '#4CAF50' });
      }
      _buttons.push({ id: 'levels', text: '关卡列表', x: cx + 8, y: popupBottom, w: btnW, h: btnH, color: '#1E88E5' });
    } else {
      _buttons.push({ id: 'retry', text: '重新挑战', x: cx - btnW - 8, y: popupBottom, w: btnW, h: btnH, color: '#FF6B35' });
      _buttons.push({ id: 'levels', text: '返回', x: cx + 8, y: popupBottom, w: btnW, h: btnH, color: '#78909C' });
    }
  }
...
/**
 * 抓火鸡 — 收藏/图鉴场景（微信小游戏 Canvas 版）
 * 展示已收集的10种火鸡，未收集显示为剪影/锁定
 */
...
class CollectionScene extends Scene {
  ...
  render(ctx, w, h) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(collected + '/10 已收集', w / 2, 62);
  ...
  _drawCard(ctx, card) {
    ...
    if (unlocked) {
      ...
      ctx.font = '10px sans-serif';
      ctx.fillText('通关对应关卡解锁', x + cw / 2, y + ch - 24);
    }
  }
...
class SettingsScene extends Scene {
  ...
  _layout() {
    ...
    _items = [
      { id: 'sfx', label: '🔊 音效', key: 'sfx', x: cx - itemW / 2, y: startY, w: itemW, h: itemH },
      { id: 'bgm', label: '🎵 背景音乐', key: 'bgm', x: cx - itemW / 2, y: startY + (itemH + gap), w: itemW, h: itemH },
    ...
    _confirmButtons = [
      { id: 'confirmYes', text: '确认重置', x: cx - confirmW - 10, y: confirmY, w: confirmW, h: 44, color: '#E53935' },
      { id: 'confirmNo', text: '取消', x: cx + 10, y: confirmY, w: confirmW, h: 44, color: '#78909C' },
    ];
  }
  ...
  render(ctx, w, h) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('🗑️ 重置存档', _resetBtn.x + _resetBtn.w / 2, _resetBtn.y + _resetBtn.h / 2);
    ...
    ctx.fillText('🦃 抓火鸡 v2.0', w / 2, h - 30);
  ...
  _drawConfirmDialog(ctx, w, h) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    ctx.fillText('⚠️ 确认重置存档？', w / 2, py + 20);

    ctx.font = '14px sans-serif';
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.fillText('所有进度和收集将被清除！', w / 2, py + 55);
